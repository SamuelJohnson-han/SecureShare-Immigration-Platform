...
class GroupFactoryMock {
  ...
  isVerifiedAuthority(principal: string): Result<boolean> {
    return this.authorities.has(principal) ? { ok: true, value: true } : { ok: false, value: ERR_NOT_AUTHORIZED };
  }
  ...
  createGroup(
    ...
    if (!this.isVerifiedAuthority(this.caller).ok) return { ok: false, value: ERR_NOT_AUTHORIZED };
  ...
  updateGroup(id: number, updateName: string, updateMaxMembers: number, updateContribAmount: number): Result<boolean> {
    ...
    if (!group) return { ok: false, value: ERR_GROUP_NOT_FOUND };
    if (group.creator !== this.caller) return { ok: false, value: ERR_NOT_AUTHORIZED };
    if (!updateName || updateName.length > 100) return { ok: false, value: ERR_INVALID_UPDATE_PARAM };
    if (updateMaxMembers <= 0 || updateMaxMembers > 50) return { ok: false, value: ERR_INVALID_MAX_MEMBERS };
    if (updateContribAmount <= 0) return { ok: false, value: ERR_INVALID_CONTRIB_AMOUNT };
    if (this.state.groupsByName.has(updateName) && this.state.groupsByName.get(updateName) !== id) {
      return { ok: false, value: ERR_GROUP_ALREADY_EXISTS };
    }
